...
router.get('/singlesummary/:state_id', async (req, res) => {
    ...
    try {
        const results = await axios.get(
            `${BASE_URL}/v2/state/${state}.json?apiKey=${actnow_config.apiKey}`
        );
...
router.get('/singleseries/:state_id', async (req, res) => {
    ...
    try {
        const results = await axios.get(
            `${BASE_URL}/v2/state/${state}.timeseries.json?apiKey=${actnow_config.apiKey}`
        );
        res.status(200).json(results.data);
    } catch (error) {
        console.log(error);
...
router.get('/series', async (req, res) => {
    try {
        const results = await axios.get(
            `${BASE_URL}/v2/states.timeseries.json?apiKey=${actnow_config.apiKey}`
        );
        res.status(200).json(results.data);
    } catch (error) {
        console.log(error);
